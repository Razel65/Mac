...
if ($conn->connect_error) {
    die(json_encode(['success' => false, 'error' => 'Database connectie mislukt: ' . $conn->connect_error]));
}
...
if ($action === 'register') {
    ...
    if (!$stmt) {
        echo json_encode(['success' => false, 'error' => 'Query voorbereiding mislukt: ' . $conn->error]);
        exit;
    }
    ...
    if ($stmt->execute()) {
        ...
        echo json_encode(['success' => false, 'error' => 'Registratie mislukt: ' . $stmt->error]);
    }
...
elseif ($action === 'update') {
    ...
    if ($stmt->execute()) {
        ...
        echo json_encode(['success' => false, 'error' => 'Bijwerken voortgang mislukt: ' . $stmt->error]);
    }
